...
- [X] T001 Configurar endpoint canônico (`http://100.75.210.114:8000`) e timeouts da API Finance Organizer em `src/test/resources/config/environments.json`
- [X] T002 [P] Criar diretórios de payloads e schemas em `src/test/resources/data/payloads/auth/` e `src/test/resources/data/schemas/auth/`
- [X] T003 [P] Implementar geradores de senhas de fronteira (8, 72, 7 e 73 caracteres) em `src/test/java/utils/DataGenerator.java`
...
- [X] T004 [P] Implementar schema de contrato `UserResponse` com Karate fuzzy matchers em `src/test/resources/data/schemas/auth/user-response-schema.json`
- [X] T005 [P] Implementar schema de contrato `Token` com Karate fuzzy matchers em `src/test/resources/data/schemas/auth/token-schema.json`
- [X] T006 [P] Implementar schema de contrato `HTTPValidationError` com validação de array `detail` em `src/test/resources/data/schemas/auth/validation-error-schema.json`
- [X] T007 [P] Criar template de payload base de cadastro em `src/test/resources/data/payloads/auth/register-request.json`
- [X] T008 [P] Criar template de payload base de login em `src/test/resources/data/payloads/auth/login-request.json`
...
- [X] T009 [US1] Criar arquivo de feature com Background e tags `@auth` e `@register` em `src/test/java/features/auth/register.feature`
- [X] T010 [P] [US1] Implementar cenários felizes SCEN-REG-01 (senha de 8 chars) e SCEN-REG-02 (senha de 72 chars) validando `UserResponse` em `src/test/java/features/auth/register.feature`
- [X] T011 [P] [US1] Implementar cenários negativos de fronteira de senha SCEN-REG-05 (7 chars) e SCEN-REG-06 (73 chars) validando HTTP 422 em `src/test/java/features/auth/register.feature`
- [X] T012 [P] [US1] Implementar cenários de validação de formato SCEN-REG-03 (e-mail malformado), SCEN-REG-07 (campos ausentes) e SCEN-REG-08 (tipagem incorreta) validando HTTP 422 em `src/test/java/features/auth/register.feature`
- [X] T013 [US1] Implementar cenário de conflito cadastral SCEN-REG-04 validando HTTP 409 Conflict para e-mail duplicado em `src/test/java/features/auth/register.feature`
- [X] T014 [US1] Implementar cenário de proteção contra Mass Assignment SCEN-REG-09 validando HTTP 422 para injeção de campos extras em `src/test/java/features/auth/register.feature`
...
- [X] T015 [US2] Criar arquivo de feature com Background e tags `@auth` e `@login` em `src/test/java/features/auth/login.feature`
- [X] T016 [US2] Implementar cenário de sucesso SCEN-LOG-01 cadastrando usuário e validando emissão de token JWT contra `token-schema.json` em `src/test/java/features/auth/login.feature`
- [X] T017 [P] [US2] Implementar cenários negativos de credenciais SCEN-LOG-02 (senha incorreta) e SCEN-LOG-03 (e-mail inexistente) validando HTTP 401 Unauthorized em `src/test/java/features/auth/login.feature`
- [X] T018 [P] [US2] Implementar cenário de segurança SCEN-LOG-04 para conta inativa (`is_active: false`) validando HTTP 403 Forbidden em `src/test/java/features/auth/login.feature`
- [X] T019 [P] [US2] Implementar cenários de validação de payload SCEN-LOG-05 (campos vazios ou nulos) validando HTTP 422 em `src/test/java/features/auth/login.feature`
...
- [X] T020 [US3] Criar arquivo de feature com Background e tags `@auth` e `@me` em `src/test/java/features/auth/me-get.feature`
- [X] T021 [US3] Implementar cenário feliz SCEN-ME-01 com Bearer token válido, validando contrato `user-response-schema.json` e ausência de hash de senha em `src/test/java/features/auth/me-get.feature`
- [X] T022 [P] [US3] Implementar cenário de ausência de cabeçalho Authorization SCEN-ME-02 validando HTTP 401 Unauthorized em `src/test/java/features/auth/me-get.feature`
- [X] T023 [P] [US3] Implementar cenário de cabeçalho Authorization malformado SCEN-ME-03 (sem prefixo Bearer) validando HTTP 401 Unauthorized em `src/test/java/features/auth/me-get.feature`
- [X] T024 [P] [US3] Implementar cenários de token adulterado ou expirado SCEN-ME-04 validando HTTP 401 Unauthorized em `src/test/java/features/auth/me-get.feature`
...
- [X] T025 [US4] Criar arquivo de feature com Background e tags `@auth` e `@delete` em `src/test/java/features/auth/me-delete.feature`
- [X] T026 [US4] Implementar cenário simples de exclusão imediata SCEN-DEL-01 para usuário sem vínculos validando HTTP 204 No Content em `src/test/java/features/auth/me-delete.feature`
- [X] T027 [US4] Implementar cenário de limpeza em cascata SCEN-DEL-02 (setup com contas, transações e investimentos), disparo de DELETE e validação de inacessibilidade dos IDs órfãos em `src/test/java/features/auth/me-delete.feature`
- [X] T028 [P] [US4] Implementar validação pós-deleção de bloqueio de login subsequente SCEN-DEL-03 validando HTTP 401 em `src/test/java/features/auth/me-delete.feature`
- [X] T029 [P] [US4] Implementar validação pós-deleção de rejeição de token revogado SCEN-DEL-04 validando HTTP 401 em `src/test/java/features/auth/me-delete.feature`
- [X] T030 [P] [US4] Implementar cenário de tentativa de exclusão sem autorização SCEN-DEL-05 validando HTTP 401 em `src/test/java/features/auth/me-delete.feature`
...
- [X] T031 Atualizar filtros de execução e paralelismo de 3 threads em `src/test/java/features/TestRunner.java`
- [X] T032 [P] Configurar mascaramento de headers de autorização e senhas em `src/test/resources/logback-test.xml`
- [X] T033 Executar suíte completa `mvn test -Dkarate.tags="@auth"` e verificar relatório HTML em `target/karate-reports/karate-summary.html` per `specs/001-auth-user-management/quickstart.md`
